document.addEventListener("DOMContentLoaded", async () => {
        ...
        li.innerHTML = `
            <div class="task-header">
                <div class="task-info" onclick="toggleAccordion(${task.id})">
                    <strong>${isChild?'<i class="fas fa-level-up-alt fa-rotate-90 sub-task-icon"></i>':''} ${task.title} ${subtaskBadge} ${overdueBadge}</strong>
                    <div class="task-meta">${dateText ? `<i class="far fa-calendar-alt"></i> <span style="margin-right:5px;">${dateText}</span>` : ''} ${recurDisplay}</div>
                </div>
                <div class="actions">
    ...
    // ===================================================
    // 👇 YENİ: YUXARIDA BƏRKİDİLMİŞ HEADLER (Dates & Subtask Btn)
    // ===================================================
