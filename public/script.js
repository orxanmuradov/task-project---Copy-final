...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // TASK FORM SUBMIT (RECURRENCE ƏLAVƏ EDİLDİ)
    document.getElementById("task-form").addEventListener("submit", async (e) => {
        e.preventDefault();
        let title = document.getElementById("task-input").value; 
        let category = categorySelect.value;
        let startDate = document.getElementById("task-start-date").value;
        let dueDate = document.getElementById("task-due-date").value;
        let recurrence = document.getElementById("task-recurrence").value; // Yeni hissə
        ...
        const res = await fetch("/api/tasks", { 
            ...
            body: JSON.stringify({ 
                ...
                recurrence: recurrence || null, // Yeni hissə
                parent_id:null 
            }) 
        });
    ...
    window.toggleStatus = async (id,s,r,t,c) => { 
        ...
        if(ns==='completed' && r && r!=='null'){ // Status completed olanda və təkrar olanda yeni task yaradırıq
            ...
            await fetch("/api/tasks",{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${token}`},body:JSON.stringify({title:t,category:c,description:"",due_date:nextDate.toISOString().split('T')[0],recurrence:r,parent_id:null})}); 
        } 
