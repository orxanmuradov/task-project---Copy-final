...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('sw.js')
      .then((reg) => { console.log('SW ok'); }, (err) => { console.log('SW error'); });
  });
}
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    categorySelect.addEventListener("change", () => { 
        newCatContainer.style.display = (categorySelect.value === "new_category") ? "block" : "none"; 
    });

    // Tapşırıq yaratmaq (Təkrar dəstəyi ilə)
    document.getElementById("task-form").addEventListener("submit", async (e) => {
        e.preventDefault();
        let title = document.getElementById("task-input").value; 
        let category = categorySelect.value;
    ...
    async function loadTasks() {
        ...
        allTasksCache.filter(t => !t.parent_id).forEach(task => {
    ...
    function renderTask(task, listElement) {
        ...
        // Təkrarolunma ikonu
        let recurIcon = task.recurrence ? `<span title="Təkrarlanan" style="color: #ffcc00; margin-left: 8px;"><i class="fas fa-sync-alt"></i></span>` : "";

        li.innerHTML = `
            <div class="task-header">
                <div class="task-info">
                    <strong>${task.title} ${recurIcon}</strong>
                </div>
                <div class="actions">
                    <button onclick="toggleStatus(${task.id},'${task.status}','${task.recurrence}','${task.title}','${task.category}')" class="check-btn">
    ...
    // Təkrar məntiqinin real işlədiyi hissə
    window.toggleStatus = async (id, s, r, t, c) => { 
        const ns = s === 'completed' ? 'pending' : 'completed'; 
        
        // Əgər tapşırıq bitirsə və təkrarı varsa, yeni task yaradılır
        if(ns === 'completed' && r && r !== 'null'){ 
            ...
            await fetch("/api/tasks", {
                method: "POST",
                headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                body: JSON.stringify({ title: t, category: c, due_date: nextDate.toISOString().split('T')[0], recurrence: r, parent_id: null })
            }); 
        } 
