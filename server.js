...
app.post("/api/tasks", authenticateToken, async (req, res) => {
  ...
  try {
    const result = await sql`
      INSERT INTO tasks (user_id, title, category, description, due_date, parent_id, recurrence, recurrence_end) 
      VALUES (${req.user.id}, ${title}, ${category || 'general'}, ${description}, ${due_date}, ${parent_id || null}, ${recurrence || null}, ${recurrence_end || null}) 
      RETURNING *
    `;
...
app.put("/api/tasks/:id", authenticateToken, async (req, res) => {
    ...
    try {
        await sql`UPDATE tasks SET title=${title}, description=${description}, due_date=${due_date}, recurrence=${recurrence}, recurrence_end=${recurrence_end} WHERE id=${req.params.id} AND user_id=${req.user.id}`;
